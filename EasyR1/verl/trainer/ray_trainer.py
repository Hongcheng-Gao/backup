...
def top_k_stateful_loader(orig_sdl: StatefulDataLoader, k=10, **override_kwargs):
    dataset = getattr(orig_sdl, "dataset", None)
    if dataset is None:
        raise ValueError("StatefulDataLoader 未暴露 dataset，无法基于索引子集构造。")
    k = min(k, len(dataset))
    subset = Subset(dataset, list(range(k)))

    # 显式使用顺序采样，确保未 shuffle
    sampler = SequentialSampler(subset)

    # 继承一些常见参数，必要时可覆盖
    default_kwargs = dict(
        batch_size=getattr(orig_sdl, "batch_size", 1),
        drop_last=False,
        num_workers=getattr(orig_sdl, "num_workers", 0),
        pin_memory=getattr(orig_sdl, "pin_memory", False),
        persistent_workers=getattr(orig_sdl, "persistent_workers", False),
        collate_fn=getattr(orig_sdl, "collate_fn", None),
        # 不用 shuffle，改用显式 sampler
        shuffle=False,
        sampler=sampler,
    )
    default_kwargs.update(override_kwargs)

    # 注意：StatefulDataLoader 的构造签名与 torch.utils.data.DataLoader 基本兼容
    new_sdl = StatefulDataLoader(subset, **default_kwargs)
    return new_sdl
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # print(f"batch_size = {self.val_dataloader_train.batch_size}", flush=True)
        # print("subset_samples =", len(self.val_dataloader_train.dataset))
        # print("=== Inspect first 10 samples (unshuffled, original order) ===", flush=True)
        # shown = 0
        # for bi, batch in enumerate(self.val_dataloader_train):
        #     if isinstance(batch, (list, tuple)) and len(batch) == 2:
        #         x, y = batch
        #         bs = len(x)
        #         for i in range(bs):
        #             if shown >= 10: break
        #             print(f"idx={shown:02d} (batch {bi}, item {i}) -> x={x[i]}, y={y[i]}", flush=True)
        #             shown += 1
        #     elif isinstance(batch, dict):
        #         bs = next((len(v) for v in batch.values() if hasattr(v, "__len__")), 1)
        #         for i in range(bs):
        #             if shown >= 10: break
        #             one = {k: (v[i] if hasattr(v, "__getitem__") and hasattr(v, "__len__") and len(v) > i else v)
        #                 for k, v in batch.items()}
        #             print(f"idx={shown:02d} (batch {bi}, item {i}) -> {one}", flush=True)
        #             shown += 1
        #     else:
        #         print(f"idx={shown:02d} (batch {bi}) -> {batch}", flush=True)
        #         shown += 1
        #     if shown >= 10:
    ...
    def _validate_train(self) -> dict[str, Any]:
        reward_tensor_lst = []
        # Lists to collect samples for the table
        sample_inputs, sample_outputs, sample_labels, sample_scores = [], [], [], []
        reward_metrics_lst = defaultdict(list)
        ...
        print("Start validation...")
        self.actor_rollout_ref_wg.prepare_rollout_engine()
        for batch_dict in self.val_dataloader_train:
            test_batch = DataProto.from_single_dict(batch_dict)
            test_gen_batch = test_batch.pop(
                batch_keys=["input_ids", "attention_mask", "position_ids"],
                non_tensor_batch_keys=["raw_prompt_ids", "multi_modal_data"],
        ...
        self.actor_rollout_ref_wg.release_rollout_engine()
        self._maybe_log_val_generations(sample_inputs, sample_outputs, sample_labels, sample_scores)
        self.val_reward_score_train = torch.cat(reward_tensor_lst, dim=0).sum(-1).mean().item()
        val_reward_metrics = {f"val(train set)/{key}_reward": value for key, value in reduce_metrics(reward_metrics_lst).items()}
        val_length_metrics = {f"val(train set)_{key}": value for key, value in reduce_metrics(length_metrics_lst).items()}
        print("Finish validation.")
